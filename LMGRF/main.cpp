...
int main(int argc, char** argv)
{
	...
	m_Dim1 = nOrder * 24 * nSampling / 4 * nMultiRegion;  //MultiLGOP
	m_Dim2 = nOrder * 24 * nSampling / 4 * nMultiRegion;  //MultiLGDOP
	m_Dim3 = nOrder * 24 * nSampling / 4 * nMultiRegion;  //MultiLIDOP
	...
	{
		int *desc = 0;
		desc = Extract_MLMGRF(m_pKeys[i], m_pImg, nOrder, nSampling, nMultiRegion);
		if ( !desc )	continue;
